...
(function (global, factory) {
	...
	var lib = createCommonjsModule(function (module, exports) {
	  ...
	  const withEditorContext = function (WrappedComponent) {
	    const InjectEditorContext = props => {
	      const {
	        forwardRef,
	        ...rest
	      } = props;
	      return React$1.createElement(EditorContext.Consumer, null, context => React$1.createElement(WrappedComponent, Object.assign({
	        ref: forwardRef
	      }, rest, context)));
	    };

	    return React$1.forwardRef((props, ref) => React$1.createElement(InjectEditorContext, Object.assign({
	      forwardRef: ref
	    }, props)));
	  };

	  const EditorPrivateContext = React$1.createContext({});
	  ...
	  const withEditorPrivateContext = function (WrappedComponent) {
	    const InjectEditorContext = props => {
	      ...
	      return React$1.createElement(EditorPrivateContext.Consumer, null, context => React$1.createElement(WrappedComponent, Object.assign({
	        ref: forwardRef
	      }, rest, context)));
	    };
	  ...
	  class GGEditor extends React$1.Component {
	    constructor(props) {
	      super(props);
	    ...
	    bindEvent(graph) {
	      ...
	      addListener(graph, EditorEvent.onGraphStateChange, ({
	        graphState
	      }) => {
	      ...
	      addListener(graph, EditorEvent.onLabelStateChange, ({
	    ...
	    render() {
	      ...
	      return React$1.createElement(EditorContext.Provider, {
	        value: {
	          graph,
	          executeCommand
	        }
	      }, React$1.createElement(EditorPrivateContext.Provider, {
	        value: this.state
	      }, React$1.createElement("div", Object.assign({}, pick_1(this.props, ['className', 'style'])), children)));
	    }
	  ...
	  class EditableLabel extends React$1.PureComponent {
	    constructor() {
	      super(...arguments);
	      ...
	      this.handleKeyDown = e => {
	        ...
	        if (key === 'Enter' || key === 'Escape') {
	          // 阻止事件冒泡
	          e.stopPropagation(); // 执行模型更新

	          this.executeUpdate(); // 隐藏编辑标签
	      ...
	      this.getLabelOffset = ({
	        ...
	        const {
	          x: left,
	          y: top
	        } = graph.getCanvasByPoint(absoluteX, absoluteY);
	        return {
	          top,
	          left
	        };
	    ...
	    componentDidUpdate() {
	      ...
	      if (labelState === LabelState.Show) {
	        // focus
	        this.labelElement.focus(); // select all
	  ...
	  commandManager.register('redo', { ...command,
	  ...
	  commandManager.register('undo', { ...command,
	  ...
	  class NodePopover extends React$1.Component {
	    constructor() {
	      super(...arguments);
	    ...
	    render() {
	      ...
	      return React$1.createElement("div", Object.assign({}, pick_1(this.props, ['style', 'className']), {
	  ...
	  class EditorGraph extends React$1.Component {
	    constructor(props) {
	      super(props);
	    ...
	    initGraph() {
	      ...
	      const {
	        clientWidth = 0,
	        clientHeight = 0
	      } = document.getElementById(containerId) || {}; // 解析数据
	    ...
	    render() {
	      ...
	      return React$1.createElement("div", Object.assign({
	        id: containerId
	      }, pick_1(this.props, ['className', 'style'])), children, React$1.createElement(EditableLabel$1, null), React$1.createElement(NodePopover$1, null));
	    }
	  ...
	  function handleAnchor(name, value, item) {
	    const model = item.getModel(); // 拿到 group

	    const group = item.getContainer(); // 拿到所有的锚点

	    const anchors = group.get('children').filter(e => e.get('className') === 'anchor'); // 非拖拽状态

	    if (!this.addingEdge && !item.hasState('addingSource')) {
	      // 进入锚点激活锚点, value 为目标锚点
	      // 离开锚点则清除所有锚点激活样式
	      if (name === 'activeAnchor') value ? value.setActived && value.setActived() : anchors.forEach(a => a.clearActived()); // 进入节点状态和选中状态显示所有锚点
	      // 离开节点隐藏所有锚点

	      if (name === 'active') value ? drawAnchor.call(this, model, group) : !item.hasState('selected') && anchors.forEach(a => a.remove());
	      if (name === 'selected' && !value) anchors.forEach(a => a.remove());
	    } else {
	      // 拖拽状态下激活锚点则激活 hotspost 样式
	      if (name === 'activeAnchor') value ? value.setHotspotActived && value.setHotspotActived(true) : anchors.forEach(a => a.setHotspotActived && a.setHotspotActived(false));
	    } // 进入拖拽状态


	    if (name === 'addingEdge') if (value) {
	      this.addingEdge = true;
	      const anchors = drawAnchor.call(this, model, group); // 拖拽状态下显示 hotspost

	      anchors.forEach(a => a.showHotspot());
	    } else {
	      // 结束拖拽时清除所有锚点
	      item.getContainer().get('children').filter(i => i.get('className') === 'anchor').forEach(a => a.remove());
	  ...
	  function drawAnchor(model, group) {
	    const anchorPoints = this.getAnchorPoints(); // 为每个点添加标记
	    ...
	    return anchorPoints.map((p, index) => {
	      ...
	      shape.setHotspotActived = bool => {
	        if (hotspot) {
	          if (bool) hotspot.attr(anchorHotsoptActivedStyle);else hotspot.attr(anchorHotsoptStyle);
	        }
	      };
	  ...
	  var upperFirst_1 = upperFirst; // import each from '@antv/util/lib/each';
	  ...
	  const Util = {
	    optimizeMultilineText(text, font, maxWidth = 94) {
	      ...
	      for (const char of text) {
	        const {
	          width
	        } = UtilCanvasContext.measureText(char);
	    ...
	    itemStates({
	      ...
	      const getCustomInitialStyle = child => {
	        ...
	        if (typeof this[`get${upperFirst_1(child.get('className'))}Style`] === 'function') {
	          const customStyle = this[`get${upperFirst_1(child.get('className'))}Style`]({
	            model: item.getModel()
	          });
	          return { ...child.getDefaultAttrs(),
	            ...customStyle,
	      ...
	      const dynamicBase = type => {
	        ...
	        Object.keys(newStyleObj).forEach(className => {
	          ...
	          if (currentChild) {
	            currentChild.attr({ ...getCustomInitialStyle(currentChild),
	      ...
	      const staticState = () => {
	        ...
	        groupChildren.map(child => {
	          return child.attr({ ...getCustomInitialStyle(child)
	          });
	        });
	        /* this.adjustKeyShape({
	          updatedKeyShape: group.findByClassName('keyShape'),
	  ...
	  const bizOption = {
	    ...
	    drawLabel(model, group) {
	      const labelDefaultStyle = this[`get${ShapeClassName.Label}defaultStyle`](); // draw label
	    ...
	    update(nextModel, item) {
	      ...
	      let label = group.findByClassName(ShapeClassName.Label); // repaint label
	    ...
	    setState(name, value, item) {
	      this.setStateStyle(item); // this.adjustPosition({ item });
	    },
	    ...
	    adjustPosition({
	      ...
	      const keyShapeSize = this.adjustKeyShape({
	        label,
	        keyShape
	      });
	    ...
	    adjustWrapper({
	      ...
	      if (!model) return; // keyShape has stroke with 1 width, so make wrapper's height plus 1
	    ...
	    setStateStyle(item) {
	      ...
	      allChildren.forEach(shape => {
	        ...
	        statesArr.forEach(stateName => {
	          statesStyle = { ...statesStyle,
	            ...(this[`get${className}${stateName}Style`] && this[`get${className}${stateName}Style`]())
	          };
	        });
	        shape.attr({ ...(this[`get${className}defaultStyle`] && this[`get${className}defaultStyle`]()),
	          ...statesStyle
	        });
	      });
	    },
	  ...
	  G6.registerNode('biz-flow-node', {
	    ...
	    update(nextModel, item) {
	      const group = item.getContainer(); // repaint anchor
	  ...
	  const {
	    ...
	    nodeSelectedStyle,
	    startNodeStyle
	  } = globalStyle; // 选中时改变边框颜色

	  function drawActivedNode(name, value, item) {
	    const revertStyle = () => {
	      const shape = item.getKeyShape().baseType; // 恢复原本样式
	    ...
	    if ((name === 'selected' || name === 'active' || name === 'activeAnchor') && value) item.get('keyShape').attr({ ...nodeActivedStyle
	    }); // 选中时加阴影

	    if (name === 'selected' && value) item.get('keyShape').attr({ ...nodeSelectedStyle
	    }); // 取消选中复原

	    if (name === 'selected' && !value) revertStyle(); // 离开节点且为非选中状态、非拖拽状态
	  ...
	  const {
	    startNodeStyle: startNodeStyle$1
	  } = globalStyle;
	  ...
	  function angleTo(v1, v2, direct) {
	    ...
	    if (direct) {
	      if (angleLargeThanPi) return Math.PI * 2 - angle;else return angle;
	    } else {
	      if (angleLargeThanPi) return angle;else return Math.PI * 2 - angle;
	    }
	  } // v1 v2 夹角
	  ...
	  function drawLabel(item, group, keyShape) {
	    ...
	    const textBox = labelShape.getBBox(); // 绘制 Label 背后的框

	    group.addShape('rect', {
	      attrs: { ...edgeLabelRectStyle,
	  ...
	  function drawActivedEdges(name, value, item) {
	    ...
	    const revertStyle = () => {
	      ...
	      keyShape.endArrow && keyShape.endArrow.attr({
	        fill: edgeStyle$1.stroke
	      });
	    }; // 选中时且鼠标停留时显示样式


	    if ((name === 'selected' || name === 'active') && value) {
	      if (keyShape.endArrow) keyShape.endArrow.attr({
	        fill: edgeActivedStyle.stroke
	      });
	      keyShape.attr({ ...edgeActivedStyle,
	        ...(name === 'selected' ? {
	          lineWidth: 2
	        } : {})
	      });
	    ...
	    if (name === 'active' && !value && !item.hasState('selected')) revertStyle(); // 线条拖拽过程中问题

	    if (name === 'drag' && value) {
	      keyShape.attr({ ...edgeDragStyle
	      });
	      keyShape.endArrow && keyShape.endArrow.attr({
	        fill: edgeDragStyle.stroke
	      });
	    }
	  ...
	  function filterConnectPoints(points) {
	    ...
	    Object.keys(pointsMap).forEach(k => result.push(pointsMap[k])); // Util.each(pointsMap, p => {
	    //     result.push(p);
	    // });
	  ...
	  G6.registerEdge('polyline', {
	    ...
	    draw(item, group) {
	      ...
	      keyShape.endArrow = drawArrow(item, group, keyShape, path); // 画 Label
	    ...
	    getPathByPoints(points, source, target) {
	      const polylinePoints = getPolylinePoints(points[0], points[points.length - 1], source, target, this.offset); // FIXME default
	  ...
	  G6.registerEdge('polylineRound', {
	    ...
	    getPathByPoints(points, source, target) {
	      const polylinePoints = simplifyPolyline(getPolylinePoints(points[0], points[points.length - 1], source, target, this.offset)); // FIXME default
	  ...
	  function getCubicControlPoints(from, to, source, target) {
	    return [source && source.getBBox() ? _getOffsetPoint(source, from, to, target) : _getStraightOffsetPoint(from, to), target && target.getBBox() ? _getOffsetPoint(target, to, from, source) : _getStraightOffsetPoint(to, from)];
	  }
	  /**
	  ...
	  function getCubicBezierCurve(points, source, target) {
	    ...
	    const M = ['M', start.x, start.y]; // automatically get the rest two control points
	  ...
	  G6.registerEdge('flowSmooth', {
	    draw(item, group) {
	      const path = this.getPath(item); // 绘制线条
	    ...
	    setState(name, value, item) {
	      // 线条激活状态
	      drawActivedEdges.call(this, name, value, item); // 线条高亮态
	  ...
	  const activeEdgeBehavior = {
	    ...
	    setAllItemStates(e) {
	      if (!this.shouldBegin(e)) return; // 1.激活当前选中的边

	      const {
	        graph
	      } = this;
	      const edge = e.item;
	      graph.setItemState(edge, 'active', true); // 2. 激活边关联的 sourceNode 与 targetNode
	    ...
	    clearAllItemStates(e) {
	      if (!this.shouldBegin(e)) return; // 状态还原
	    ...
	    onDrag(e) {
	      ...
	      nodes.forEach(node => {
	        const {
	          horizontalLines,
	          verticalLines
	        } = this.getBoxLine(node); // 1. 计算水平或垂直方向相关的三个对齐点到节点水平线之间的距离
	        // 2. 一旦距离小于误差便自动吸附，一旦吸附就终止其它对齐点的计算
	        // 3. 吸附后对齐点到水平线的距离应该是无误差的，此时显示对齐线

	        const calc = (points, lines, arr, axis) => {
	          start: for (const p of points) for (const name of Object.keys(lines)) {
	            ...
	            if (Math.abs(dis) < tolerance) {
	              ...
	              if (axis === 'Y') delegateShape.attr({
	                y: originPoint.y - dis
	              });
	              if (axis === 'X') delegateShape.attr({
	                x: originPoint.x + dis
	              });
	              arr[name].push({
	                line,
	      ...
	      const drawLine = (lines, d) => {
	        ...
	        names.forEach(name => {
	          if (lines[name].length > 0) {
	            ...
	            const y = line[1];
	            const x = line[0]; // 将画布转标转换为视口坐标并乘缩放比例
	  ...
	  const dragAddEdge = {
	    ...
	    isAnchor(e) {
	      ...
	      if (targetName == 'anchor') return true;else return false;
	    },
	    ...
	    shouldBegin(e) {
	      ...
	      const targetName = target.get('className'); // 如果点击的不是锚点就结束
	    ...
	    onMousedown(e) {
	      ...
	      graph.getNodes().forEach(node => {
	        if (node.get('id') !== node.get('id')) graph.setItemState(node, 'addingEdge', true);else graph.setItemState(node, 'addingSource', true);
	      });
	      const point = {
	        x: e.x,
	        y: e.y
	      };
	      const model = node.getModel(); // 如果在添加边的过程中，再次点击另一个节点，结束边的添加
	      // 点击节点，触发增加边
	  ...
	  const dragNode = {
	    ...
	    shouldBegin(e) {
	      // 锚点上不触发拖拽；
	      if (e.target.get('className') == 'anchor') return false;else return true;
	    },
	    ...
	    onDragStart(e) {
	      ...
	      this.target = e.item; // 单节点拖拽当做多节点拖拽的特例
	    ...
	    onDragEnd(e) {
	      ...
	      selectedNodes.map(item => item.get('delegateShape')).forEach(ds => {
	        if (ds) {
	          ['HTL', 'HCL', 'HBL', 'VLL', 'VCL', 'VRL'].forEach(lname => {
	    ...
	    _update(e, force) {
	      ...
	      if (showDelegate && !force) {
	        ...
	        if (selectedNodes.length > 1 && !this.multipleDelegate) this.drawMultipleDelegate(); // this._updateDelegate(item, x, y);
	        // 更新所有委托图形的位置;
	    ...
	    _updateDelegate(item, x, y) {
	      ...
	      if (!shape) {
	        ...
	        const attrs = delegateStyle; // model上的x, y是相对于图形中心的，delegateShape是g实例，x,y是绝对坐标
	      ...
	      const isAttach = () => shape.attachFlag === 'Y' && Math.abs(y - shape.attr('y')) <= shape.attachTolerance || shape.attachFlag === 'X' && Math.abs(x - shape.attr('x')) <= shape.attachTolerance;
	      ...
	      if (isAttach()) {
	        if (shape.attachFlag === 'Y') setAttr({
	          x
	        });
	        if (shape.attachFlag === 'X') setAttr({
	          y
	        });
	      } else {
	        setAttr({
	  ...
	  const brushSelect = {
	    ...
	    onMouseDown(e) {
	      if (this.keyFlag) return; // 按在node上面拖动时候不应该是框选
	  ...
	  class Flow extends React$1.Component {
	    constructor() {
	      super(...arguments);
	    ...
	    render() {
	      ...
	      return React$1.createElement(Graph, Object.assign({
	        containerId: containerId,
	        data: data,
	        parseData: parseData,
	        initGraph: initGraph
	      }, pick_1(this.props, ['className', 'style'])));
	    }
	  ...
	  var index = withEditorPrivateContext(Flow);
	  const options = { ...bizOption,
	    ...
	    drawExpandOrCollapseButton(model, group) {
	      const keyShape = group.findByClassName(ShapeClassName.KeyShape); // button width

	      const width = 17; // button & item distance
	      ...
	      const button = group.addShape('path', {
	        className: ShapeClassName.CollapseExpandButton,
	        attrs: {
	          path: Util.getCollapseButtonPath({
	            width,
	            height: width
	          }),
	          ...this[`get${ShapeClassName.CollapseExpandButton}defaultStyle`](),
	  ...
	  const options$1 = {
	    draw(model, group) {
	      ...
	      if (startNode.getModel().y === endNode.getModel().y && endNode.getModel().x < 0) {
	        return group.addShape('path', {
	          attrs: {
	            path: [['M', startX - sourceWidth, startY + sourceHeight / 2], ['L', endX + wrapperOffset, endY + targetHeight / 2]],
	            ...commonStyle
	          }
	        });
	      }
	    ...
	    drawLeftBottom(group, startX, startY, endX, endY, sourceHeight, targetHeight, targetWidth, sourceWidth) {
	      ...
	      return group.addShape('path', {
	        attrs: {
	          path: [['M', startX - sourceWidth, startY + sourceHeight / 2], ['L', startX - sourceWidth - offset + radius, startY + sourceHeight / 2], ['A', radius, radius, 0, 0, 0, startX - sourceWidth - offset, startY + sourceHeight / 2 + radius], ['L', endX + offset, endY + targetHeight / 2 - radius], ['A', radius, radius, 0, 0, 1, endX + offset - radius, endY + targetHeight / 2], ['L', endX + wrapperOffset, endY + targetHeight / 2]],
	          ...commonStyle
	        }
	      });
	    },
	    ...
	    drawLeftTop(group, startX, startY, endX, endY, sourceHeight, targetHeight, targetWidth, sourceWidth) {
	      ...
	      return group.addShape('path', {
	        attrs: {
	          path: [['M', startX - sourceWidth, startY + sourceHeight / 2], ['L', startX - sourceWidth - offset + radius, startY + sourceHeight / 2], ['A', radius, radius, 0, 0, 1, startX - sourceWidth - offset, startY + sourceHeight / 2 - radius], ['L', endX + offset, endY + targetHeight / 2 + radius], ['A', radius, radius, 0, 0, 0, endX + offset - radius, endY + targetHeight / 2], ['L', endX + wrapperOffset, endY + targetHeight / 2]],
	          ...commonStyle
	        }
	      });
	    },
	    ...
	    drawRightBottom(group, startX, startY, endX, endY, sourceHeight, targetHeight, sourceWidth) {
	      ...
	      return group.addShape('path', {
	        attrs: {
	          path: [['M', startX, startY + sourceHeight / 2], ['L', startX + sourceWidth + offset - radius, startY + sourceHeight / 2], ['A', radius, radius, 0, 0, 1, startX + sourceWidth + offset, startY + sourceHeight / 2 + radius], ['L', endX - offset, endY + targetHeight / 2 - radius], ['A', radius, radius, 0, 0, 0, endX - offset + radius, endY + targetHeight / 2], ['L', endX - wrapperOffset, endY + targetHeight / 2]],
	          ...commonStyle
	        }
	      });
	    },
	  ...
	  G6.registerEdge('biz-mind-edge', options$1);
	  const commonStyle$1 = {
	    stroke: '#d8d8d8',
	    lineWidth: 2
	  };
	  ...
	  G6.registerEdge('biz-cubic', options$2, 'cubic-horizontal');
	  const topicCommand = { ...baseCommand,
	    ...
	    execute(graph) {
	      ...
	      const parent = graph.findById(id).get('parent'); // 添加节点

	      graph.addChild(model, parent); // 选中节点

	      this.setSelectedNode(graph, model.id); // 编辑节点
	  ...
	  const subtopicCommand = { ...topicCommand,
	    ...
	    execute(graph) {
	      ...
	      const parent = graph.findById(id); // 添加节点
	  ...
	  const unfoldCommand = { ...foldCommand,
	  ...
	  class Mind extends React$1.Component {
	    constructor() {
	      super(...arguments);
	    ...
	    getHGap(model) {
	      ...
	      if (totalTextWidth >= 120) return 120; // 节点最小宽度
	  ...
	  var index$1 = withEditorPrivateContext(Mind);

	  class Command extends React$1.Component {
	    constructor() {
	      super(...arguments);
	      this.state = {
	        disabled: false
	    ...
	    render() {
	      ...
	      return React$1.createElement("div", {
	        className: `command${canExecuteCommand(name) ? '' : ' command-disabled'}`,
	  ...
	  var index$2 = withEditorPrivateContext(Command);
	  ...
	  class Item extends React$1.PureComponent {
	    constructor(props) {
	      ...
	      this.handleDragenter = ev => {
	        ...
	        const canvas = graph.get('container').getElementsByTagName('canvas')[0]; // drag into canvas
	      ...
	      this.handleDrag = ev => {
	        ...
	        if (dragShape) {
	          const transferredPos = graph.getPointByClient(ev.clientX, ev.clientY);
	          graph.update(dragShapeID, { ...transferredPos
	          });
	        }
	      };
	      ...
	      this.handleDrop = ev => {
	        ...
	        const transferredPos = graph.getPointByClient(ev.clientX, ev.clientY); // drag into canvas
    ...
    `;
	    ...
	    render() {
	      ...
	      }, pick_1(this.props, ['style', 'className'])), src ? React$1.createElement("img", {
	        src: src,
	        alt: shape,
	        draggable: false
	      }) : children);
	  ...
	  var Item$1 = withEditorPrivateContext(Item);

	  class ItemPanel extends React$1.PureComponent {
	    constructor(props) {
	      super(props);
	      this.bindEvent();
	    }
	    ...
	    render() {
	      ...
	      return React$1.createElement("div", Object.assign({}, pick_1(this.props, ['style', 'className'])), children);
	    }

	  }

	  class DetailPanel extends React$1.Component {
	    ...
	    render() {
	      ...
	      return React$1.createElement("div", Object.assign({}, pick_1(this.props, ['style', 'className'])), React$1.Children.toArray(children).map(child => {
	  ...
	  class ContextMenu extends React$1.Component {
	    constructor() {
	      super(...arguments);
	  ...
	  var index$3 = withEditorPrivateContext(ContextMenu);

	  class Register extends React$1.Component {
	    constructor(props, type) {
	      super(props);
	      const {
	        name,
	  ...
	  exports.default = GGEditor;
	  exports.withEditorContext = withEditorContext;
	});
	var GGEditor = unwrapExports(lib);
	var lib_1 = lib.CanvasPanel;
	var lib_2 = lib.Command;
	var lib_3 = lib.ContextMenu;
	var lib_4 = lib.EdgePanel;
	var lib_5 = lib.Flow;
	var lib_6 = lib.Item;
	var lib_7 = lib.ItemPanel;
	var lib_8 = lib.Mind;
	var lib_9 = lib.MultiPanel;
	var lib_10 = lib.NodeMenu;
	var lib_11 = lib.NodePanel;
	var lib_12 = lib.RegisterBehavior;
	var lib_13 = lib.RegisterCommand;
	var lib_14 = lib.RegisterEdge;
	var lib_15 = lib.RegisterNode;
	var lib_16 = lib.commandManager;
	var lib_17 = lib.withEditorContext;
	...
	var css = "#root {\n  display: flex;\n  width: 100%;\n  height: 100%;\n  overflow: hidden;\n}\n.editor--2U6f3 {\n  display: flex;\n  flex: 1;\n}\n.editor-bd--3ibzT {\n  flex: 1;\n}\n";
	var styles = {"editor":"editor--2U6f3","editor-bd":"editor-bd--3ibzT","editorBd":"editor-bd--3ibzT"};
	styleInject(css);
	...
	class Index extends React.Component {
	    render() {
	        return (React.createElement(GGEditor, { className: styles.editor },
	            React.createElement(lib_8, { className: styles.editorBd, data: data })));
	    }
	}
